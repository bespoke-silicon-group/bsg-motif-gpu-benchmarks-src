...
def set_NVCC_COMMAND(MAX_REG=64):
	...
	if MAX_REG == 64:	
		...
		NVCC_COMMAND += " -DNUM_CTA=" + str(CTA_STYLE[0]) + " -DCTA_SIZE=" + str(CTA_STYLE[1])
	else:
		print("Invalid MAX_REG configuration, not supported\n")
		exit(-1)

	NVCC_COMMAND += " -std=c++11 -O3 -I " + DIR_PATH+"/../.." + "/src/runtime_lib/ -Xcompiler \"-w\" -Wno-deprecated-gpu-targets --maxrregcount=" + str(MAX_REG) + " "
...
def run_ds():
	...
	DS_ROAD = "ds_road.gt"
	set_NVCC_COMMAND()
	compile_application(DS_SOCIAL, "ds_social")
	...
	compile_application(DS_ROAD, "ds_road")
...
def run_bfs():
	...
	set_NVCC_COMMAND()
	compile_application(BFS_SOCIAL, "bfs_social")
	#for i, (name, graph) in enumerate(GRAPH_SOCIAL):
	#	compile_and_run(BFS_SOCIAL, "bfs_social", graph + " 0 " + str(threshold[name]), "bfs_" + name + ".out")
	#	print(str(i+1) + "/" + str(len(GRAPH_ALL)))
	set_NVCC_COMMAND(512)
	compile_application(BFS_ROAD, "bfs_road")
	#for i, (name, graph) in enumerate(GRAPH_ROAD):
