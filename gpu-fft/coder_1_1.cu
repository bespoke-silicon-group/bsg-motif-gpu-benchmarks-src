...
float2* sdata = (float2*)shared;
	...
	mult.x = 0;
	mult.y = 0;
		if ((((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (65536) < 65536) {
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (512) + 512 * (threadIdx.y + 0) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (65536);
			inoutID = (inoutID);
			temp_0=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (512) + 512 * (threadIdx.y + 16) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (65536);
			inoutID = (inoutID);
			temp_1=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (512) + 512 * (threadIdx.y + 32) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (65536);
			inoutID = (inoutID);
			temp_2=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (512) + 512 * (threadIdx.y + 48) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (65536);
			inoutID = (inoutID);
			temp_3=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (512) + 512 * (threadIdx.y + 64) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (65536);
			inoutID = (inoutID);
			temp_4=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (512) + 512 * (threadIdx.y + 80) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (65536);
			inoutID = (inoutID);
			temp_5=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (512) + 512 * (threadIdx.y + 96) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (65536);
			inoutID = (inoutID);
			temp_6=inputs[inoutID];
		inoutID = ((threadIdx.x + blockIdx.x * blockDim.x)) % (512) + 512 * (threadIdx.y + 112) + (((threadIdx.x + blockIdx.x * blockDim.x)) / 512) * (65536);
			inoutID = (inoutID);
	...
	temp_3.y = temp_3.y + loc_0.y;
		...
		mult = twiddleLUT[91 + (((threadIdx.x + blockIdx.x * blockDim.x)) % (512)) + (threadIdx.y + 0) * 512];
		w.x = temp_0.x * mult.x - temp_0.y * mult.y;
		temp_0.y = temp_0.y * mult.x + temp_0.x * mult.y;
		temp_0.x = w.x;
		mult = twiddleLUT[91 + (((threadIdx.x + blockIdx.x * blockDim.x)) % (512)) + (threadIdx.y + 16) * 512];
		w.x = temp_1.x * mult.x - temp_1.y * mult.y;
		temp_1.y = temp_1.y * mult.x + temp_1.x * mult.y;
		temp_1.x = w.x;
		mult = twiddleLUT[91 + (((threadIdx.x + blockIdx.x * blockDim.x)) % (512)) + (threadIdx.y + 32) * 512];
		w.x = temp_2.x * mult.x - temp_2.y * mult.y;
		temp_2.y = temp_2.y * mult.x + temp_2.x * mult.y;
		temp_2.x = w.x;
		mult = twiddleLUT[91 + (((threadIdx.x + blockIdx.x * blockDim.x)) % (512)) + (threadIdx.y + 48) * 512];
		w.x = temp_3.x * mult.x - temp_3.y * mult.y;
		temp_3.y = temp_3.y * mult.x + temp_3.x * mult.y;
		temp_3.x = w.x;
		mult = twiddleLUT[91 + (((threadIdx.x + blockIdx.x * blockDim.x)) % (512)) + (threadIdx.y + 64) * 512];
		w.x = temp_4.x * mult.x - temp_4.y * mult.y;
		temp_4.y = temp_4.y * mult.x + temp_4.x * mult.y;
		temp_4.x = w.x;
		mult = twiddleLUT[91 + (((threadIdx.x + blockIdx.x * blockDim.x)) % (512)) + (threadIdx.y + 80) * 512];
		w.x = temp_5.x * mult.x - temp_5.y * mult.y;
		temp_5.y = temp_5.y * mult.x + temp_5.x * mult.y;
		temp_5.x = w.x;
		mult = twiddleLUT[91 + (((threadIdx.x + blockIdx.x * blockDim.x)) % (512)) + (threadIdx.y + 96) * 512];
		w.x = temp_6.x * mult.x - temp_6.y * mult.y;
		temp_6.y = temp_6.y * mult.x + temp_6.x * mult.y;
		temp_6.x = w.x;
		mult = twiddleLUT[91 + (((threadIdx.x + blockIdx.x * blockDim.x)) % (512)) + (threadIdx.y + 112) * 512];
